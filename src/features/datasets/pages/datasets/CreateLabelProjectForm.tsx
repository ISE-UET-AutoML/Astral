...
import { Plus as PlusIcon, X as XMarkIcon, ChevronDownIcon } from 'lucide-react'
...
const Tag = ({ variant: _variant, ...props }: any) => <Badge {...props} />
	...
	useEffect(() => {
		if (csvMetadata?.columns) {
			const options = Object.entries(csvMetadata.columns).map(([key, val]) => ({
				value: key,
				label: `${key} (${(val as any).unique_class_count ?? 0} classes)`,
				uniqueClassCount: (val as any).unique_class_count ?? 0,
			}))
