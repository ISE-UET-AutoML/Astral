...
import { Database as CircleStackIcon, Star as StarIcon, Trash as TrashIcon, Image as PhotoIcon, FileText as DocumentTextIcon, Table as TableCellsIcon, Grid2x2 as Squares2X2Icon, ChartBar as ChartBarIcon, RefreshCw as ArrowPathIcon } from 'lucide-react'
...
const resolveTypeFamily = (key: string): string => {
    if (!key) return 'TEXT'
...
export default function DatasetCard({ dataset, onDelete, isDeleting: _isDeleting }: DatasetCardProps) {
    const handleDelete = (e: MouseEvent<HTMLButtonElement>, datasetID: string) => {
        e.preventDefault()
        e.stopPropagation()
    ...
    const statusConfig = PROCESSING_STATUS[processingStatus as DatasetStatus] || PROCESSING_STATUS.PROCESSING
    ...
    const TypeIcon = (TYPE_ICON_MAP[familyKey as DatasetTypeKey] || TYPE_ICON_MAP[normalizedKey as DatasetTypeKey] || CircleStackIcon) as IconComponent
    const accentColor = TYPE_ACCENT[familyKey] || '#3B82F6'

    const handleCardClick = () => {
        if (isCompleted && lsProjectId) {
            window.open(`${import.meta.env.VITE_LABEL_STUDIO_URL}/projects/${lsProjectId}`, '_blank')
        } else if (!isCompleted) {
