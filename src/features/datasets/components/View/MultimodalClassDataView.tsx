...
const MultimodalClassDataView = ({ dataset, files }) => {
  ...
  const colsName = Object.values(files?.[0] || {}) as string[];
      ...
      <table className="min-w-full h-max border-collapse overflow-y-auto">
        ...
        <tbody>
          {currentRows.map((row, index) => (
            <tr key={index} className="hover:bg-gray-100 border-2 border-solid">
              <td className="px-2 py-2 text-center">
                <input
                  type="checkbox"
                  className="cursor-pointer"
                  checked={selectedRows.includes(indexOfFirstRow + index)}
