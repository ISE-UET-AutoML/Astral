...
type DatasetImageFile = {
  fileName: string;
  content: string;
};

type UploadDirectoryInputProps = InputHTMLAttributes<HTMLInputElement> & {
...
const ImageClassDataView = ({ dataset, files }) => {
  ...
  const [imageFiles] = useState<DatasetImageFile[]>(files || []);
  ...
  const handleFileChange = (event) => {
    if (dataset.type) {
      const files = Array.from(event.target.files || []) as File[]; // Change FileList to Array
      ...
      setTotalKbytes(Number(totalSizeInKB));
    }
  };
  ...
  const uploadNewFiles = async (event) => {
    ...
    for (let i = 0; i < uploadedfiles.length; i++) {
      ...
      let fileNameBase64 = window.btoa(uploadedfiles[i].webkitRelativePath);
    ...
    try {
      ...
      if (response.status === 201) {
        setIsLoading(false);
        toast.success("Successfully Add new Files", { duration: 3000 });
      }
    } catch (error) {
      setIsLoading(false);
      toast.error("Add new Files Fail", { duration: 3000 });
    }
  };
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      const response = await datasetAPI.deleteObjects(dataset._id, formData);
      if (response.status === 200) {
        toast.success("Successfully Delete Objects", { duration: 3000 });
      }
    } catch (error) {
      toast.error("Error deleting objects", { duration: 3000 });
    }
  };
          ...
          currentImages.map((file, index) => (
            ...
            >
              <input
                ...
                onChange={() => handleSelectImage(file.fileName)}
          ...
          >
            <div className="w-full h-full bg-white p-5  cursor-pointer rounded-[15px]">
              <div className="flex flex-col">
                ...
                <p className="text-center text-[15px] font-[300]">
                  {(dataset.type && TASK_TYPES[dataset.type]?.description) ||
            ...
            <div className="w-full h-full bg-white p-5 cursor-pointer rounded-[15px]">
              <div className="flex flex-col">
                ...
                <p className="text-center text-[15px] font-[300]">
                  Folder information will be automatically tagged as metadata to
                  each media{" "}
                </p>
              </div>
            </div>
            ...
            <div className="max-h-[90px] overflow-y-auto w-full transition-opacity duration-100">
              {uploadedfiles.map((file) => (
                ...
                >
                  ...
                  <button
                    className="ml-auto"
                    onClick={() => handleDeleteFile(file.webkitRelativePath)}
                  >
                    <XMarkIcon
                      className="h-5 w-5 text-gray-400 mr-3"
                      aria-hidden="true"
