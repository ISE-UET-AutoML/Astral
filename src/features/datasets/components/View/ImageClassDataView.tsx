	...
	const uploadNewFiles = async (event) => {
		...
		try {
			...
			if (response.status === 201) {
				setIsLoading(false)
				toast.success('Successfully Add new Files', 3)
			}
		} catch (error) {
			setIsLoading(false)
			toast.error('Add new Files Fail', 3)
		}
	}
	...
	const handleDeleteImage = async () => {
		...
		try {
			...
			if (response.status === 200) {
				toast.success('Successfully Delete Objects', 3)
			}
		} catch (error) {
			toast.error('Error deleting objects', 3)
		}
	}
