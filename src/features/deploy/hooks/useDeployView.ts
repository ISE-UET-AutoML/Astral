...
import { getDeployData } from 'src/features/deploy/api/deploy'
...
import * as visualizeAPI from 'src/features/models/api/visualize'
...
export const useDeployView = ({ deployId, projectId }) => {
	...
	const handleViewPrediction = async (prediction) => {
		...
		try {
			...
			toast.error('Failed to load prediction content. Please try again.')
