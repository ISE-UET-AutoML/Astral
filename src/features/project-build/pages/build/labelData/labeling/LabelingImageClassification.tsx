...
import { trainModel, autoLabel } from 'src/features/projects/api/project'
...
const LabelingImageClassification = ({
		...
		(config, index) => {
			...
			const onUpdate = async (ls, annotations) => {
				...
				if (at.length > 0)
					...
					toast.error('Please annotating', 3)
					return
				}
				...
				increase(index)
				toast.success('Successfully Updated', 3)
			}
			...
			const onSubmit = async (ls, annotations) => {
				...
				console.log('result update', resultUpdate)
				increase(index)
				toast.success('Successfully Submitted', 3)
			}
	...
	const checkData = () => {
		...
		if (currentLabeled.size !== fullLabel.size) {
			toast.error('you must label more')
			return false
		}
		return true
	}
	...
	const handleAutoLabeling = async () => {
		...
		if (result.status === 200) {
			toast.success('auto label successfully')
			window.location.reload()
			return
		}
		toast.error("Can't auto labeling", result)
		setIsLoading(false)
	}
