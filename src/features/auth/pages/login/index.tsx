...
import * as auth from 'src/features/auth/api/auth'
import useAuth from 'src/features/auth/hooks/useAuth'
import { validateEmail, validatePassword } from 'src/utils/validate'
import { PATHS } from 'src/constants/paths'
import TextCubeCanvas from 'src/features/auth/components/login/TextCubeCanvas'
import LoginCard from 'src/features/auth/components/login/LoginCard'
...
const RESPONSIVE_SIZES = {
	...
	default: { minWidth: 0, sizeHalf: 200, offsetX: -240 },
}
...
const loginToLabelStudio = async (lsToken: string) => {
	const labelStudioBaseUrl =
		import.meta.env.VITE_LABEL_STUDIO_URL || 'http://127.0.0.1:8080'
...
const Login = () => {
	...
	const onLogin = async (credential: LoginCredential) => {
		try {
			const { data } = await auth.login(credential) as { data: LoginResponse }
