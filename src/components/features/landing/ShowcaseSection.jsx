						...
						{showcaseItems.map((item, index) => (
							...
							>
								...
								>
									...
									{/* Glow effect */}
									<div 
										className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-20 transition-opacity duration-500 pointer-events-none bg-gradient-to-tl from-[#5C8DFF] to-[#65FFA0] blur-[20px] scale-110"
									/>
								</div>

								{/* Reflection effect */}
								<div 
									className="absolute top-full left-0 right-0 h-20 opacity-0 group-hover:opacity-30 transition-opacity duration-500 pointer-events-none bg-gradient-to-b from-white/10 to-transparent" style={{ transform: 'rotateX(180deg) translateY(-1px)' }}
								/>
							</div>
						))}
					</div>
