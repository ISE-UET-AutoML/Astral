...
export default function AstralCanvas({
    ...
    useEffect(() => {
        ...
        const rings = [
            { rx: 1.00, ry: 0.28, rz: 0.95, tilt: 0.00, phase: 0,              count: 80, speed: 0.0004 },
            { rx: 0.85, ry: 0.85, rz: 0.28, tilt: Math.PI * 0.33, phase: 1.1,  count: 70, speed: 0.0006 },
            { rx: 0.28, ry: 0.95, rz: 0.88, tilt: Math.PI * 0.62, phase: 2.3,  count: 60, speed: 0.0003 },
        ]
        ...
        function onMouseMove(e) {
            const r = container.getBoundingClientRect()
            tYaw   = ((e.clientX - r.left - r.width  / 2) / (r.width  / 2)) * mouseMaxYaw
            tPitch = ((e.clientY - r.top  - r.height / 2) / (r.height / 2)) * mouseMaxPitch
        }
        ...
        function scale() { return Math.min(W, H) / 520 }

        function project(x, y, z, R, CAM_Z, FOCAL) {
            const [px, py, pz] = mulMV(R, [x, y, z])
            const z2 = pz + CAM_Z
            return { x: FOCAL * px / z2 + cx, y: FOCAL * py / z2 + cy, z: z2, pz }
        }
        ...
        function draw(t) {
            ...
            const ambient = ctx.createRadialGradient(cx, cy, 0, cx, cy, RADIUS * 1.1)
            ...
            floaters.forEach(f => {
                ...
                const twk = 0.6 + 0.4 * Math.sin(t * 0.001 * f.twinkleSpeed + f.twinkle)
                const al  = f.alpha * twk * Math.max(0.1, Math.min(1, 500 / p.z * 0.9))
            ...
            rings.forEach((ring, ri) => {
                ...
                const projected = pts3d.map(p => project(p.x, p.y, p.z, R, CAM_Z, FOCAL))
                ...
                for (let i = 0; i < ring.count; i++) {
                    ...
                    const al   = Math.max(0.03, Math.min(0.30, 480 / avgZ * 0.25))
                ...
                for (let i = 0; i < ring.count; i += step) {
                    ...
                    // Halo
                    const g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, 6 * sc)
        ...
        if (!prefersReducedMotion) requestAnimationFrame(loop); else draw(0)
    ...
    }, [offsetX, baseSpinYaw, mouseMaxYaw, mouseMaxPitch, followEasing, prefersReducedMotion])
