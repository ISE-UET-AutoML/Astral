...
const StatusBadge = ({ status }) => {
	const statusConfig = {
		...
		running: { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-800 dark:text-blue-300', label: 'Running' },
		generating: { bg: 'bg-purple-100 dark:bg-purple-900/30', text: 'text-purple-800 dark:text-purple-300', label: 'Generating' },
		generated: { bg: 'bg-indigo-100 dark:bg-indigo-900/30', text: 'text-indigo-800 dark:text-indigo-300', label: 'Generated' },
		deploying: { bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-800 dark:text-orange-300', label: 'Deploying' },
		deployed: { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-800 dark:text-green-300', label: 'Deployed' },
...
function AppCard({ app, onViewDetails }) {
	...
	const frontendUrl = hasInstance && app?.ports?.frontend 
		...
		<Card className="rounded-2xl shadow-lg bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:shadow-xl transition-all duration-200 hover:-translate-y-1">
			<CardHeader className="pb-3">
				<div className="flex items-start justify-between gap-2">
					<div className="flex items-center gap-2 flex-1 min-w-0">
						...
						<CardTitle className="text-lg text-gray-900 dark:text-white truncate" title={app?.name}>
							{app?.name || `App #${app?.id}`}
						</CardTitle>
					</div>
			...
			<CardContent className="space-y-3">
				...
				<div className="flex items-center gap-2 text-sm">
					<span className="text-gray-500 dark:text-gray-400 font-medium">Task:</span>
				...
				{app?.model_id != null && (
					<div className="flex items-center gap-2 text-sm">
						<span className="text-gray-500 dark:text-gray-400 font-medium">Model ID:</span>
				...
				{app?.error_message && (
					<div className="mt-2 p-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
						<p className="text-xs text-red-600 dark:text-red-400 line-clamp-2" title={app.error_message}>
							{app.error_message}
						</p>
					</div>
				)}
