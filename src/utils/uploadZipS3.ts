 ...
 */
const parseAndValidateFiles = (files: ParsedUploadFile[]) => {
	const validFiles: ParsedUploadFile[] = []
	const labels: string[] = []
 ...
 * @param {File[]} files - Array of File objects
 */
const uploadZippedFilesToS3 = async (presignedUrl: string, files: ParsedUploadFile[]) => {
	const zip = new JSZip()
...
async function uploadFilesToS3(presignedUrls: Record<string, string>, files: File[]) {
	...
	for (const file of files) {
		...
		try {
			...
			console.error(`Lỗi khi upload ${file.name}:`, error.response?.data || error.message);
			results.push({
				fileName: file.name,
				status: "error",
