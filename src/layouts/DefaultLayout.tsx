...
const handleCompletedDataset = async (
  item: PendingLabelProject,
  removingPending: RemovePending,
) => {
  ...
  try {
    ...
    console.log(
      `Dataset '${dataset.title}' is COMPLETED. Creating label project...`,
    );
    await labelProjectAPI.createLbProject(payload);
  } catch (err) {
    console.error(
      `Error while creating label project '${dataset.title}':`,
      err,
    );
  } finally {
...
const LabelProjectPollingManager = () => {
